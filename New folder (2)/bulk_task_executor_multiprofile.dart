...
class BulkTaskExecutor {
  ...
  Future<void> _executeMultiProfileTask(BulkTask task) async {
    ...
    // Determine concurrency limit (only relaxed/Lower Priority models use 4, fast uses 10)
    final isRelaxedModel = task.model.contains('relaxed') || 
                            task.model.contains('Lower Priority') ||
                            task.model.contains('Relaxed');
    final maxConcurrent = isRelaxedModel ? 4 : 20;
    print('[CONCURRENCY] Model: "${task.model}", IsRelaxed: $isRelaxedModel, MaxConcurrent: $maxConcurrent');
