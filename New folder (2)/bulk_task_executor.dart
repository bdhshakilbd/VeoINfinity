    ...
    for (var i = 0; i < scenesToProcess.length; i++) {
      ...
      while (!success && (scene.retryCount ?? 0) < maxRetries && task.status == TaskStatus.running) {
        ...
        try {
          ...
          // Random delay between API requests (3-5 seconds)
          final delay = 3 + _random.nextInt(3); // 3, 4, or 5 seconds
          print('[DELAY] Waiting ${delay}s before request');
  ...
  Future<void> _generateWithProfile(
    ...
    if (result['status'] != null && result['status'] != 200) {
      ...
      if (statusCode == 403) {
        ...
        // Handle token refresh for Mobile profiles - trigger auto-relogin for THIS browser only
        if ((Platform.isAndroid || Platform.isIOS) && 
          ...
          _mobileService!.autoReloginProfile(
            ...
            password: _password,
            onSuccess: () {
              print('[403] ${profile.name} relogin SUCCESS');
            }
          );
        }
      }
  ...
  Future<void> _processBatchPollingQueue(BulkTask task, int maxConcurrent) async {
    ...
    while ((task.status == TaskStatus.running || task.status == TaskStatus.paused) && 
      ...
      try {
        ...
        // Find generator for polling - use healthy profiles only
        dynamic pollGenerator;
        String? pollToken;

        if ((Platform.isAndroid || Platform.isIOS) && _mobileService != null) {
          // Get any healthy profile for polling
        ...
        if (pollGenerator == null || pollToken == null) {
          print('[POLLER] No healthy browser for polling');
          
          // If task is paused (waiting for relogin), wait for it to resume
          if (task.status == TaskStatus.paused) {
            print('[POLLER] Task paused, waiting for resume...');
            while (task.status == TaskStatus.paused) {
              await Future.delayed(const Duration(seconds: 2));
            }
            continue;
          ...
          // Check if this is because all browsers are relogging
          if ((Platform.isAndroid || Platform.isIOS) && _mobileService != null && _countHealthyProfiles() == 0) {
            print('[POLLER] ⏸ All browsers unhealthy - triggering relogin...');
            
            // Trigger relogin for profiles that need it
            _mobileService!.reloginAllNeeded(
              email: _email,
              password: _password,
              onAnySuccess: () {
                print('[POLLER] ✓ Browser recovered from relogin');
              },
            );
            
            // Wait until at least one browser becomes healthy (max 30s for polling to resume)
            int waitAttempts = 0;
            while (_countHealthyProfiles() == 0 && waitAttempts < 6 && (task.status == TaskStatus.running || task.status == TaskStatus.paused)) {
              await Future.delayed(const Duration(seconds: 5));
              waitAttempts++;
              print('[POLLER] Waiting for browser... (${waitAttempts * 5}s, Healthy: ${_countHealthyProfiles()})');
            }
            
            if (_countHealthyProfiles() > 0) {
              print('[POLLER] ✓ Browser available, resuming polling...');
            } else {
              print('[POLLER] ⚠ Still no healthy browser after 30s - continuing anyway...');
            }
          } else {
