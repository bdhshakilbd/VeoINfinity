...
class BulkTaskExecutor {
  ...
  Future<void> _executeTaskMultiBrowser(BulkTask task) async {
    ...
    final maxConcurrent = 999; // No artificial limit - let API tell us when to slow down
    print('[CONCURRENCY] ===============================');
    ...
    print('[CONCURRENCY] Strategy: ${isRelaxedModel ? "Relaxed (429-throttled)" : "Fast (unlimited)"}');
  ...
  Future<void> _processGenerationQueueMultiBrowser(
    ...
    for (var i = 0; i < scenesToProcess.length; i++) {
      ...
      // IMMEDIATE RETRY LOOP - cycles through browsers up to 5 times for errors
      // Matches VideoGenerationService retry strategy
      final maxRetries = 5;
      bool success = false;
      ...
      while (!success && (scene.retryCount ?? 0) < maxRetries && task.status == TaskStatus.running) {
        ...
        try {
          ...
          // Retryable error (403, 429, etc.) - retry with different browser after delay
          scene.retryCount = (scene.retryCount ?? 0) + 1;
          
          // Check if this is a 429 error - wait 30s
          int retryDelay = 3 + _random.nextInt(3); // Default 3-5s
          if (e.message.contains('429') || e.message.contains('quota')) {
            retryDelay = 30; // Wait 30s for quota to refresh
          ...
          if (scene.retryCount! >= maxRetries) {
            ...
            _onTaskStatusChanged?.call(task);
            break; // Exit retry loop - scene has failed
          }
  ...
  Future<void> _generateWithProfile(
    ...
    if (scene.firstFrameMediaId != null || scene.lastFrameMediaId != null) {
      ...
      if (actualModel.contains('_t2v_')) {
        ...
        if (scene.firstFrameMediaId != null && scene.lastFrameMediaId != null) {
          // Both frames: use _i2v_s_fast_fl_ pattern
          actualModel = actualModel.replaceFirst('_t2v_fast_', '_i2v_s_fast_fl_');
          actualModel = actualModel.replaceFirst('_t2v_quality_', '_i2v_s_quality_fl_');
        } else {
    ...
    if (statusCode == 403 || errorStr.contains('recaptcha')) {
      ...
      print('[403 DETECTED] ${profile.name} - Count: ${profile.consecutive403Count}/7');
      
      // UPDATED STRATEGY: Use 3 consecutive 403s threshold (matches VideoGenerationService)
      if (profile.consecutive403Count < 7) {
        print('[403 STRATEGY] Continuing with current session (${7 - profile.consecutive403Count} attempts left)...');
      } else {
        print('[403 STRATEGY] Threshold reached → Triggering relogin');
        ...
        if (profile is ChromeProfile && 
          ...
          print('[403 RELOGIN] ${profile.name} - Triggering relogin...');
  ...
  if (errorMsg.contains('429') || errorMsg.contains('Resource has been exhausted') || errorMsg.contains('check quota')) {
    print('[429] Quota exhausted - setting 30s cooldown');
    _last429Time = DateTime.now();
    _activeGenerations[task.id] = (_activeGenerations[task.id] ?? 1) - 1;
    print('[SLOT] Released slot (429 quota) - Active: ${_activeGenerations[task.id]}');
    throw _RetryableException('429 quota exhausted - will wait 30s before retry');
  }
  
  // Check for reCAPTCHA error (treat like 403 - session issue)
  final errorStr = errorMsg.toLowerCase();
  if (errorStr.contains('recaptcha')) {
    profile.consecutive403Count++;
    print('[403 DETECTED] ${profile.name} - reCAPTCHA error - Count: ${profile.consecutive403Count}/7');
    
    // Trigger relogin after 3 consecutive reCAPTCHA failures
    if (profile.consecutive403Count >= 7) {
      print('[403 STRATEGY] Threshold reached → Triggering relogin');
      print('[403 DEBUG] Profile type: ${profile.runtimeType}');
      print('[403 DEBUG] loginService: ${_loginService != null ? "SET" : "NULL"}');
      print('[403 DEBUG] mobileService: ${_mobileService != null ? "SET" : "NULL"}');
      
      bool reloginTriggered = false;
      
      // Handle relogin for Chrome profiles
      if (profile is ChromeProfile && _loginService != null) {
        print('[403 RELOGIN] ${profile.name} - Triggering Chrome relogin...');
        // Pass email/password - reloginProfile will get from Settings if empty
        _loginService!.reloginProfile(profile, _email, _password);
        profile.consecutive403Count = 0;
      ...
      // Handle token refresh for Mobile profiles
      if ((Platform.isAndroid || Platform.isIOS) && profile is MobileProfile && _mobileService != null) {
        print('[403 RELOGIN] ${profile.name} - Triggering Mobile relogin...');
        final mobileProfile = profile as MobileProfile;
      ...
      if (!reloginTriggered) {
        print('[403 RELOGIN] ✗ Relogin NOT triggered - loginService or mobileService is NULL');
        print('[403 RELOGIN] ✗ Resetting counter to prevent infinite loop');
        profile.consecutive403Count = 0;
      }
    }
