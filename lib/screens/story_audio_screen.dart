...
class _StoryAudioScreenState extends State<StoryAudioScreen> {
            ...
            appBar: AppBar(
              ...
              title: const Row(
                children: [
                  ...
                  SizedBox(width: 8),
                  Flexible(child: Text('Bulk Reels/Shorts Create', overflow: TextOverflow.ellipsis)),
                ],
              ),
              ...
              actions: [
                ...
                if (Platform.isAndroid || Platform.isIOS)
                  IconButton(
                    icon: Icon(_mobileBrowserVisible ? Icons.close : Icons.open_in_browser),
                    tooltip: _mobileBrowserVisible ? 'Close Browser' : 'Open Browser (Login)',
                    onPressed: () {
                      setState(() => _mobileBrowserVisible = !_mobileBrowserVisible);
                    },
                  ),
                IconButton(
  ...
  Widget _buildReelTab() {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        // Desktop: Split panel layout (controls left, content right)
        if (isDesktop) {
          return Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              // Left Panel - Controls (flex 1.5/4 = ~37.5%)
              Expanded(
                flex: 150, // 1.5 ratio
                child: Container(
              ...
              // Right Panel - Generated Reels (flex 2.5/4 = ~62.5%)
              Expanded(
                flex: 250, // 2.5 ratio
                child: Container(
                  height: constraints.maxHeight,
                  child: SingleChildScrollView(
                    child: Padding(
                      padding: EdgeInsets.all(padding),
                      child: _buildReelsListPanel(padding, false),
                    ),
                  ),
                ),
              ),
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildReelsListPanel(padding, isMobile),
              ],
            ),
          ),
        );
  ...
  Widget _buildControlsPanel(double padding, bool isMobile) {
    return Column(
      ...
      children: [
        ...
        Card(
          child: Padding(
            padding: EdgeInsets.all(isMobile ? padding : 8),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                if (isMobile) _buildOutputFolderSection(), // Only show on mobile
                ...
                const SizedBox(height: 8),
                    ...
                    isMobile
                      ...
                      : Column(
                          crossAxisAlignment: CrossAxisAlignment.stretch,
                          children: [
                            // Template Cards Row with Add Button
                            Row(
                              children: [
                              ...
                              child: ListView(
                                ...
                                children: [
                                  ...
                                  ..._state.reelTemplates.map((t) => _buildTemplateCard(
                                    id: t.id,
                                    name: t.name,
                                    imagePath: null, // No image for custom templates
                    ...
                    Row(
                      children: [
                        const Text('Character:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
                        const SizedBox(width: 4),
                        Expanded(
                          child: TextFormField(
                        ...
                        const SizedBox(width: 8),
                        const Text('Voice Over Language:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 10)),
                        const SizedBox(width: 4),
                        Expanded(
                          child: Autocomplete<String>(
                            ...
                            onSelected: (value) {
                              setState(() {
                                ...
                                _voiceOverLanguageController.text = value;
                                _state = _state.copyWith(reelLanguage: value);
                              });
                              _saveState();
                            },
                            fieldViewBuilder: (context, controller, focusNode, onSubmitted) {
                              // Sync controller with our tracked value on first build
                              if (controller.text.isEmpty || controller.text == 'English') {
                    ...
                    Row(
                      children: [
                        ...
                        SegmentedButton<String>(
                          segments: const [
                            ButtonSegment(value: 'single', label: Text('Single Topic', style: TextStyle(fontSize: 11))),
                            ButtonSegment(value: 'multi', label: Text('One per Line', style: TextStyle(fontSize: 11))),
                          ],
                          ...
                          : Wrap(
                              ...
                              children: [
                                Row(
                                  ...
                                  children: [
                                    const Text('Reels:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
                                    const SizedBox(width: 4),
                                    SizedBox(
                                      width: 50,
                                      child: TextField(
                                ...
                                Row(
                                  mainAxisSize: MainAxisSize.min,
                                  children: [
                                    const Text('Stories/Hint:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
                                    const SizedBox(width: 4),
                                    DropdownButton<int>(
                                      value: _storiesPerHint,
                                      isDense: true,
                                ...
                                Row(
                                  mainAxisSize: MainAxisSize.min,
                                  children: [
                                    const Text('Voice Cue:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
                                    Switch(
                        ...
                        Padding(
                          ...
                          child: Row(
                             children: [
                                const Text('Voice Cue Language:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
                                const SizedBox(width: 8),
                                Expanded(
                                   child: Autocomplete<String>(
                          ...
                          children: [
                            Expanded(
                              child: ElevatedButton.icon(
                                ...
                                label: Text(_isGeneratingReel ? 'Generating...' : 'Generate Content', style: const TextStyle(fontSize: 13)),
                                style: ElevatedButton.styleFrom(
  ...
  Widget _buildReelsListPanel(double padding, bool isMobile) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Wrap(
                        ...
                        children: [
                          ...
                          Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              const Text('Export:', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold)),
                              const SizedBox(width: 4),
                          ...
                          Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              const Text('Res:', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold)),
                              const SizedBox(width: 4),
                              DropdownButton<String>(
                          ...
                          Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              const Text('Auto Export:', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold)),
                              Switch(
                                value: _bulkAutoExport,
                                onChanged: (v) => setState(() => _bulkAutoExport = v),
                                materialTapTargetSize: MaterialTapTargetSize.shrinkWrap,
                              ),
                            ],
                          ),
                          ...
                          Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              const Text('Voice:', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold)),
                              const SizedBox(width: 4),
                              DropdownButton<String>(
                      ...
                      const SizedBox(height: 4),
                      // Volume Sliders Row (separate row for better visibility)
                      Row(
                        children: [
                          // TTS Volume Slider
                          const Text('Voice Vol:', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold)),
                          Expanded(
                            child: Slider(
                          ...
                          SizedBox(
                            width: 40,
                            child: Text('${(_exportTtsVolume * 100).toInt()}%', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold)),
                          ),
                          const SizedBox(width: 16),
                          // Video Audio Volume Slider
                          const Text('Vid Vol:', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold)),
                          Expanded(
                            child: Slider(
                          ...
                          SizedBox(
                            width: 40,
                            child: Text('${(_exportVideoVolume * 100).toInt()}%', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold)),
                          ),
                        ],
                      ),
                      const SizedBox(height: 8),
                      ...
                      Row(
                        children: [
                          const Text('Global Voice Style:', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold)),
                          const SizedBox(width: 4),
                          const Tooltip(
                            message: 'When set, this style will be used for ALL reels instead of AI-generated styles',
                          ...
                          Expanded(
                            child: TextFormField(
                              ...
                              decoration: InputDecoration(
                                hintText: 'e.g., Warm and engaging storytelling voice...',
                               ...
                               Row(
                                 children: [
                                   ...
                                   ElevatedButton.icon(
                                     ...
                                     style: ElevatedButton.styleFrom(
                                       backgroundColor: _isRegeneratingMissing ? Colors.grey.shade200 : Colors.orange.shade100,
                                       foregroundColor: _isRegeneratingMissing ? Colors.grey : Colors.orange.shade900,
                                            ...
                                            child: Column(
                                              ...
                                              children: [
                                                ...
                                                ElevatedButton.icon(
                                                  ...
                                                  label: const Text('Gen Audio', style: TextStyle(fontSize: 11)),
                                                ),
                                                const SizedBox(height: 4),
                                                ElevatedButton.icon(
                                                  onPressed: () => _generateReelVideo(index), 
                                                  icon: const Icon(Icons.video_library, size: 16),
                                                  label: const Text('Gen Video', style: TextStyle(fontSize: 11)),
                                                ),
                                                const SizedBox(height: 4),
                                                ElevatedButton.icon(
                                                ...
                                                Container(
                                                  ...
                                                  child: Row(
                                                    children: [
                                                      ...
                                                      const SizedBox(width: 6),
                                                      const Expanded(child: Text('Scene Prompts', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12))),
                                                      TextButton.icon(
                                                        onPressed: () => _toggleMuteAll(index, true),
                                                        icon: const Icon(Icons.volume_off, size: 12),
                                                        label: const Text('Mute All', style: TextStyle(fontSize: 9)),
                                                        style: TextButton.styleFrom(padding: EdgeInsets.zero, minimumSize: const Size(50, 24)),
                                                      ),
                                                      TextButton.icon(
                                                        onPressed: () => _toggleMuteAll(index, false),
                                                        icon: const Icon(Icons.volume_up, size: 12),
                                                        label: const Text('Unmute', style: TextStyle(fontSize: 9)),
                                                        style: TextButton.styleFrom(padding: EdgeInsets.zero, minimumSize: const Size(50, 24)),
                                                      ),
                                                    ],
                                                  ),
  ...
  Widget _buildTemplateCard({
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Column(
          children: [
            // Image container with 9:16 ratio
            Container(
              width: 80,
              height: 107,
              decoration: BoxDecoration(
  ...
  Future<void> _processReelFull(int index) async {
     ...
     // 3. Export (only if auto export is enabled AND all videos are ready)
     if (_autoProcessMap[projectId] == true && _bulkAutoExport) {
        // Check if all videos are completed
        final content = project['content'] as List?;
        bool allVideosReady = true;
        int missingCount = 0;
        ...
        if (content != null) {
          for (var part in content) {
            ...
            if (visuals != null) {
              for (var visual in visuals) {
                final status = visual['gen_status'];
                final videoPath = visual['video_path'];
                
                if (status != 'completed' || videoPath == null || !await File(videoPath).exists()) {
        ...
        if (allVideosReady) {
          print('[AUTO EXPORT] All videos ready - starting export...');
          await _exportReelWithSettings(index, _bulkExportMethod, _bulkExportResolution);
        } else {
          print('[AUTO EXPORT] ✗ Skipping export - $missingCount videos missing or failed');
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('Export skipped: $missingCount videos missing or failed. Please regenerate missing videos and export manually.'),
...
- Make it feel like a fresh, original story while keeping the core theme""";
  ...
  Future<void> _generateReelVideo(int projectIndex) async {
     ...
     print('[REEL VIDEO] Task created: ${task.id}');
     print('[REEL VIDEO] Starting bulk executor...');
     
     // Start
     try {
       await _bulkExecutor.startTask(task);
       print('[REEL VIDEO] ✓ Bulk executor started successfully');
     } catch (e, stackTrace) {
       print('[REEL VIDEO] ✗ Bulk Task Start Error: $e');
        ...
        child: Column(
          ...
          children: [
            ...
            if (_state.videosPaths != null && _state.videosPaths!.isNotEmpty) ...[
              const SizedBox(height: 8),
              Text(
                '✓ ${_state.videosPaths!.length} videos loaded',
