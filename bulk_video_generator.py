...
class BrowserVideoGenerator:
    ...
    def type_into_element(self, element_id, text, typing_speed='human'):
        """Type text into element using CDP keyboard events (triggers React state)"""
        ...
        # First, simulate a mouse click on the textarea to activate React
        # This is critical - React needs to see a real click event
        click_js = f'''
        (function() {{
            ...
            // Simulate mouse events (mouseover, mousedown, mouseup, click)
        ...
        result = self.execute_js(click_js)
        if not result or not result.get('success'):
            return {'success': False, 'error': 'Failed to click/focus element'}
        
        # Small delay after click
        time.sleep(0.3)
        
        # Type character by character using CDP keyboard events
        # This triggers real keyboard events that React detects
        for i, char in enumerate(text):
            # Send the character via CDP
            self.send_command('Input.dispatchKeyEvent', {
                'type': 'char',
                'text': char,
            })
            
            # After each character, trigger React events via JavaScript
            # This ensures React's controlled component state updates
            trigger_js = f'''
            (function() {{
                const el = document.getElementById('{element_id}');
                if (el) {{
            ...
            '''
            self.execute_js(trigger_js)
            
            # Random delay between characters (human-like typing)
            delay_ms = random.randint(base_delay[0], base_delay[1])
        ...
        try:
            ...
            (async function() {{
                try {{
                    ...
                    const token = sessionData.access_token || '{access_token}';
                    
                    // 2. Reconstruct base64 from chunks
            ...
            # Replace placeholders safely (avoid f-string brace escaping issues)
            try:
                js_code = js_code.replace("'{access_token}'", json.dumps(access_token))
            except Exception:
                pass
            js_code = js_code.replace('{chunks_js}', chunks_js)
            js_code = js_code.replace('"{mime_type}"', json.dumps(mime_type))
            js_code = js_code.replace('"{aspect_ratio}"', json.dumps(aspect_ratio))
    ...
    def generate_video(self, prompt, access_token, aspect_ratio='VIDEO_ASPECT_RATIO_LANDSCAPE', 
        ...
        # Get project ID from URL if possible to match session
        js_get_project = "(function() { const m = window.location.href.match(/project\\/([^\\/?]+)/); return m ? m[1] : null; })()"
        # Use fixed project id if not present in the page URL
        actual_project_id = self.execute_js(js_get_project) or '19d5acf8-925d-4b20-ac76-eee40b704301'
        
        js_code = f'''
        (async function() {{
            try {{
                // 1. Get Access Token fresh
                const sessionRes = await fetch('https://labs.google/fx/api/auth/session', {{credentials: 'include'}});
                const sessionData = await sessionRes.json();
                ...
                // 4. Send Request (with all browser headers from working request)
                const response = await fetch(
                    '{endpoint}',
                    {{
                        method: 'POST',
    ...
    def generate_video_ui(self, prompt, model='veo_3_1_t2v_fast_ultra'):
        """Generate video using UI automation (click, type, Enter) - browser handles reCAPTCHA automatically"""
        try:
            ...
            # Step 2: Get textarea position and click on it using CDP
            print(f"[UI-AUTO] Clicking on textarea...")
            ...
            pos = self.execute_js(get_pos_js)
            if not pos:
                print(f"[UI-AUTO] ✗ Textarea not found")
                return {'success': False, 'error': 'Textarea not found'}
            
            # Click on textarea using CDP mouse events
            x, y = pos['x'], pos['y']
            ...
            print(f"[UI-AUTO] ✓ Clicked textarea at ({x:.0f}, {y:.0f})")
            time.sleep(0.3)
            
            # Step 3: Clear existing text and type prompt character by character
            ...
            # Step 4: Wait a moment for React to process
            thinking_pause = random.uniform(0.5, 1.0)
            print(f"[UI-AUTO] Waiting {thinking_pause:.1f}s...")
            time.sleep(thinking_pause)
            
            # Step 5: Press Enter to submit
            print(f"[UI-AUTO] Pressing Enter to generate...")
            self.send_command('Input.dispatchKeyEvent', {
                'type': 'keyDown',
                'key': 'Enter',
                'code': 'Enter',
            ...
            print(f"[UI-AUTO] ✓ Enter pressed - browser handling reCAPTCHA and API call")
            
            # Step 6: Wait for generation to start
            print(f"[UI-AUTO] Waiting for generation to start...")
            time.sleep(3)
            
            # Step 7: Wait for video URL to appear in UI
